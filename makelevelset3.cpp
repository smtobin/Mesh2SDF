...
static Vec3f point_segment_direction(const Vec3f &x0, const Vec3f &x1, const Vec3f &x2)
{
   Vec3f dx(x2-x1);
   double m2=mag2(dx);
   // find parameter value of closest point on segment
   ...
   // and find the distance
   return normalized(s12*x1+(1-s12)*x2 - x0);
}

static Vec3f point_triangle_direction(const Vec3f& x0, const Vec3f& x1, const Vec3f& x2, const Vec3f& x3)
{
   // first find barycentric coordinates of closest point on infinite plane
   Vec3f x13(x1-x3), x23(x2-x3), x03(x0-x3);
   float m13=mag2(x13), m23=mag2(x23), d=dot(x13,x23);
   ...
   float w12=1-w23-w31;
   if(w23>=0 && w31>=0 && w12>=0){ // if we're inside the triangle
      Vec3f diff = w23*x1+w31*x2+w12*x3 - x0;
      if (mag(diff) < 1e-6)   // check if we're on the plane - if so use the plane normal as the gradient
      {
         Vec3f x21 = x2 - x1; Vec3f x31 = x3 - x1;
         Vec3f n = cross(x21, x31);
         return n;
      }
      return normalized(w23*x1+w31*x2+w12*x3 - x0);
      // return dist(x0, w23*x1+w31*x2+w12*x3); 
   }else{ // we have to clamp to one of the edges
      if(w23>0) // this rules out edge 2-3 for us
      {
         if (point_segment_distance(x0,x1,x2) < point_segment_distance(x0,x1,x3))
            return point_segment_direction(x0,x1,x2);
         else
            return point_segment_direction(x0,x1,x3);
      }
         // return min(point_segment_distance(x0,x1,x2), point_segment_distance(x0,x1,x3));
      else if(w31>0) // this rules out edge 1-3
      {
         if (point_segment_distance(x0,x1,x2) < point_segment_distance(x0,x2,x3))
            return point_segment_direction(x0,x1,x2);
         else
            return point_segment_direction(x0,x2,x3);

      }
         // return min(point_segment_distance(x0,x1,x2), point_segment_distance(x0,x2,x3));
      else // w12 must be >0, ruling out edge 1-2
      {
         if (point_segment_distance(x0,x1,x3) < point_segment_distance(x0,x2,x3))
            return point_segment_direction(x0,x1,x3);
         else
            return point_segment_direction(x0,x2,x3);
      }
...
void make_level_set3(const std::vector<Vec3ui> &tri, const std::vector<Vec3f> &x,
                     ...
                     Array3f &phi, Array3<Vec3f> &grads, const int exact_band)
{
   const double eps = 1e-8;
   phi.resize(ni, nj, nk);
   ...
   for(unsigned int t=0; t<tri.size(); ++t){
     ...
     // coordinates in grid to high precision
      ...
      for(int k=k0; k<=k1; ++k) for(int j=j0; j<=j1; ++j){
         if (j == 9 && k == 9)
         {
            std::cout << "j=9 and k=9!" << std::endl;
            std::cout << "(" << j << "," << k << ") in triangle (" << fjp << "," << fkp << ") (" << fjq << "," << fkq << ") (" << fjr << "," << fkr << ")" << std::endl;
         }
         double a, b, c;
         bool in_triangle = point_in_triangle_2d(j, k, fjp, fkp, fjq, fkq, fjr, fkr, a, b, c);
         if (j == 9 && k == 9)
         {
            std::cout << "barycentric coords: " << a << ", " << b << ", " << c << std::endl;
         }
         if(in_triangle){
            ...
            if ( (a-eps) <= 0 || (b-eps) <= 0 || (c-eps) <= 0)
            {
               ...
               if ( (a-eps) <= 0 && (b-eps) <= 0)  // on vertex R
               {
                  direct_hit = r;
               }
               else if ( (a-eps) <= 0 && (c-eps) <= 0)   // on vertex Q
               {
                  direct_hit = q;
               }
               else if ( (b-eps) <= 0 && (c-eps) <= 0)   // on vertex P
               {
                  direct_hit = p;
               }
               else if ( (a-eps) <= 0)   // on line QR
               {
                  direct_hit = std::min(q,r);
               }
               else if ( (b-eps) <= 0)    // on line PR
               {
                  direct_hit = std::min(p,r);
               }
               else if ( (c-eps) <= 0)    // on line PQ
               {
                  direct_hit = std::min(p,q);
               }
               else
               {
                  assert(0);
               ...
               int key = i_interval*nj*nk + j*nk + k;
               if (direct_hits.count(key) == 0)
               {
                  direct_hits[key] = std::vector<unsigned>({direct_hit});
               }
               else
               {
                  ...
                  {
                     // std::cout << "ALREADY COUNTED INTERSECTION WITH THIS EDGE/VERTEX!" << std::endl;
                     continue;
                  }
                  else
                  {
            ...
            if (j == 9 && k == 9)
            {
               std::cout << "point in triangle! j=k=" << j << std::endl;
               std::cout << "intersection count for (" << i_interval << "," << j << "," << k << "): " << intersection_count(i_interval,j,k) << std::endl;
            }
         }
         else
         {
            if (j==9 && k==9)
            {
               std::cout << "j=9 and k=9 and not in triangle!" << std::endl;
            }
         }
      }
   }
   ...
   for(int k=0; k<nk; ++k) for(int j=0; j<nj; ++j){
      ...
      for(int i=0; i<ni; ++i){
         ...
         // if (j==48 && k==48)
         //    std::cout << "(" << i << "," << j << "," << k << ") intersection count: " << total_count << std::endl;
         // if (i==9 && j==48 && k==49)
         //    std::cout << "(" << i << "," << j << "," << k << ") intersection count: " << total_count << std::endl;
   ...
   grads.resize(ni, nj, nk);
   // do gradients from closest tris
   for (int k=0; k<nk; ++k) for (int j=0; j<nj; ++j) for (int i=0; i<ni; ++i)
   {
      unsigned int p, q, r; assign(tri[closest_tri.at(i,j,k)], p, q, r);
      Vec3f gx(i*dx+origin[0], j*dx+origin[1], k*dx+origin[2]);
      Vec3f grad = point_triangle_direction(gx, x[p], x[q], x[r]);

      if (phi.at(i,j,k) < 0)  grads.at(i,j,k) = grad;
      else                    grads.at(i,j,k) = -grad;
